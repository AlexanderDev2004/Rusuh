...
import type { ManagementConfig, ManagementStatus, QueryOptions } from './types'
...
export async function ApiRequest<T>(path: string, secret: string, init?: RequestInit): Promise<T> {
  const response = await fetch(path, {
    ...init,
    headers: {
      Accept: 'application/json',
